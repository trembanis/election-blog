...
```{r turnout/vote share effects, echo=FALSE, include=FALSE, warning=FALSE}
...
clinton_plot <- clinton_fo |>
  filter(state == "WI" | state == "VA" | state == "TX" | state == "TN" | state == "PA" | state == "OH" | state == "NY" | state == "NV" | state == "NJ" | state == "NH" | state == "NC" | state == "MN" | state == "MI" | state == "ME" | state == "IN" | state == "IA" | state == "FL" | state == "CT" | state == "CO" | state == "CA" | state == "AZ") |>
  ggplot(aes(y = state)) +
  geom_bar(fill = "#7CB9E8") +
  ggtitle("Clinton Field Offices") +
  my_theme()
...
trump_plot <- trump_fo |>
  filter(state != "GA" & state != "IN" & state != "MS") |>
  ggplot(aes(y = state)) +
  geom_bar(fill = "#E8747C") +
  ggtitle("Trump Field Offices") +
  my_theme()

fig1a <- ggarrange(trump_plot, clinton_plot)
...
clinton_vs_trump <- fo_add |> filter(year == 2016 & swing16 == 1) |> ggplot(aes(y = state)) + geom_bar(aes(fill = candidate), position = "dodge") + scale_fill_manual(values = c("#7CB9E8", "#E8747C")) + xlab("Number of Field Offices") + ggtitle("Clinton vs. Trump, Swings") + my_theme()
...
fig1b <- ggarrange(clinton_vs_trump, vs_obama_both, vs_romney_both)
```
...
```{r fig 1a print, echo=FALSE, include=TRUE, warning=FALSE}
fig1a
```
...
```{r fig 1b print, echo=FALSE, include=TRUE, warning=FALSE}
fig1b
```
...
summary(tfmc_pop) # adjusted r-squared 0.7735
...
mod_lm_dem_simp <- lm(state_pv2p ~ ics + incumbent + poll_week_3 + state_pv2p_lag + mean_support,
                      data = d_train)
summary(mod_lm_dem_simp) # adjusted r-squared = 0.7575, all vars highly significant
...
sum(win_pred$harris_ec) #267 Harris EC (not counting DC)
...
figb <- ec24_pred |>
  ...
  ggtitle("Fig. B - Electoral College Prediction, 2024") +
  my_map_theme() +
...
```{r fig a print, echo=FALSE, include=TRUE, warning=FALSE}
figa
```
...
```{r fig b print, echo=FALSE, include=TRUE, warning=FALSE}
figb
```
