...
regression <- data.frame(Statistic = c("Intercept", "Index of Consumer Sentiment (C)", "National Poll Average, 1 Week Left (P)", "Candidate Was In Previous Administration (A)", "Previous Year National Two-Party Vote Share (L)", "Previous Year State Two-Party Vote Share (V)", "State Poll Average or Imputed Support, All-Time (S)"),
                        `Popular Vote` = c(15.806, 0.008, 0.674, -1.358, 0.074, "not included", "not included"),
                        `Electoral College` = c(8.105, -0.035, -0.033, -4.307, "not included", 0.600, 0.425))
...
# Non-Swing Bias
mean(non_results$real_dem - non_results$simp_pred_dem) # -1.055848

# Non-Swing MSE
mean((non_results$real_dem - non_results$simp_pred_dem)^2) # 2.939459

# Non-Swing RMSE
sqrt(mean((non_results$real_dem - non_results$simp_pred_dem)^2)) # 1.714485

# Non-Swing MAE
mean(abs(non_results$real_dem - non_results$simp_pred_dem)) # 1.393134
...
errors24 <- tibble(Metric = c("Bias", "Mean Squared Error", "Root Mean Squared Error", "Mean Absolute Error"),
...
rsquared <- tibble(Regression = c("All Variables", "No Polling Average", "No Index of Consumer Sentiment", "No Previous Administration Dummy", "No Lagged Vote Share", "Swap GDP Growth for ICS", "Swap Incumbent Party for Previous Administration"),
