...
d_state_tfmc <- d_state_tfmc |>
  mutate(
    state_poll_error = state_pv2p - poll_week_3
  )
...
pop_pred <- predict(tfmc_pop, pop24, interval = "prediction", level = 0.8) # 52.78% Harris 2-party popular vote

table <- tibble(Estimate = c("Dem Two-Party Vote Share", "Lower Bound", "Upper Bound"),
                pv2p = c(52.77931, 48.24221, 57.31641))
...
fig2 <- ec24_rescaled |>
  ...
  scale_fill_gradient2(high = "#7CB9E8", # high (positive) values represent a greater average Republican vote share
                       ...
                       limits = c(-25, 25)) +
  ggtitle("Fig. II - State-Level Vote Margin, 2024") +
  my_map_theme() +
  theme(strip.text = element_text(size = 12), 
        aspect.ratio = 0.75)
```
