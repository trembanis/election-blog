...
# Read polling data. 
d_polls <- read_csv("data/national_polls_1968-2024(7).csv")
...
summary(tfmc_dc)
dc_pred <- predict(tfmc_dc, DC24, interval = "none")
...
figx <- ec20_pred |>
  ...
  geom_polygon(aes(fill = winner), color = "#000033") +
  scale_fill_manual(values = c("#7CB9E8", "#FF6680")) +
  ggtitle("Fig. X - Electoral College Prediction, 2020") +
  my_map_theme() +
  theme(strip.text = element_text(size = 12), 
        aspect.ratio = 0.75)
...
figy <- ec20_rescaled |>
  ...
  scale_fill_gradient2(high = "#7CB9E8", # high (positive) values represent a greater average Republican vote share
                       ...
                       limits = c(-25, 25)) +
  ggtitle("Fig. Y - State-Level Vote Margin, 2020") +
  my_map_theme() +
  theme(strip.text = element_text(size = 12), 
        aspect.ratio = 0.75)
...
figza <- ec16_pred |>
  ...
  geom_polygon(aes(fill = winner), color = "#000033") +
  scale_fill_manual(values = c("#7CB9E8", "#FF6680")) +
  ggtitle("Fig. ZA - Electoral College Prediction, 2016") +
  my_map_theme() +
  theme(strip.text = element_text(size = 12), 
        aspect.ratio = 0.75)
...
figzb <- ec16_rescaled |>
  ...
  scale_fill_gradient2(high = "#7CB9E8", # high (positive) values represent a greater average Republican vote share
                       ...
                       breaks = c(-50, -25, 0, 25, 50),
                       limits = c(-25, 25)) +
  ggtitle("Fig. ZB - State-Level Vote Margin, 2016") +
  my_map_theme() +
  theme(strip.text = element_text(size = 12), 
        aspect.ratio = 0.75)
```
...
poll_comp <- tibble(State = c("Indiana", "Massachusetts", "Nebraska", "Washington"),
       `Polls Available` = c("41.47%", "65.48%", "40.19%", "59.28%"),
       `Polls Imputed` = c("41.76%", "66.95%", "40.18%", "59.79%"))

table_poll <- knitr::kable(poll_comp, caption = "Predicted Harris Two-Party Vote Share by 2024 State Poll Availability")
```
