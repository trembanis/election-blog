...
Alabama <- Alabama |> select(year, state, party, state_pv2p, ics, incumbent, poll_week_4, state_pv2p_lag, mean_support)
Alaska <- Alaska |> select(year, state, party, state_pv2p, ics, incumbent, poll_week_4, state_pv2p_lag, mean_support)
Arkansas <- Arkansas |> select(year, state, party, state_pv2p, ics, incumbent, poll_week_4, state_pv2p_lag, mean_support)
Arizona <- Arizona |> select(year, state, party, state_pv2p, ics, incumbent, poll_week_4, state_pv2p_lag, mean_support)
California <- California |> select(year, state, party, state_pv2p, ics, incumbent, poll_week_4, state_pv2p_lag, mean_support)
Colorado <- Colorado |> select(year, state, party, state_pv2p, ics, incumbent, poll_week_4, state_pv2p_lag, mean_support)
Connecticut <- Connecticut |> select(year, state, party, state_pv2p, ics, incumbent, poll_week_4, state_pv2p_lag, mean_support)
Delaware <- Delaware |> select(year, state, party, state_pv2p, ics, incumbent, poll_week_4, state_pv2p_lag, mean_support)
Florida <- Florida |> select(year, state, party, state_pv2p, ics, incumbent, poll_week_4, state_pv2p_lag, mean_support)
Georgia <- Georgia |> select(year, state, party, state_pv2p, ics, incumbent, poll_week_4, state_pv2p_lag, mean_support)
Hawaii <- Hawaii |> select(year, state, party, state_pv2p, ics, incumbent, poll_week_4, state_pv2p_lag, mean_support)
Idaho <- Idaho |> select(year, state, party, state_pv2p, ics, incumbent, poll_week_4, state_pv2p_lag, mean_support)
Illinois <- Illinois |> select(year, state, party, state_pv2p, ics, incumbent, poll_week_4, state_pv2p_lag, mean_support)
Indiana <- Indiana |> select(year, state, party, state_pv2p, ics, incumbent, poll_week_4, state_pv2p_lag, mean_support)
Iowa <- Iowa |> select(year, state, party, state_pv2p, ics, incumbent, poll_week_4, state_pv2p_lag, mean_support)
Kansas <- Kansas |> select(year, state, party, state_pv2p, ics, incumbent, poll_week_4, state_pv2p_lag, mean_support)
Kentucky <- Kentucky |> select(year, state, party, state_pv2p, ics, incumbent, poll_week_4, state_pv2p_lag, mean_support)
Louisiana <- Louisiana |> select(year, state, party, state_pv2p, ics, incumbent, poll_week_4, state_pv2p_lag, mean_support)
Maine <- Maine |> select(year, state, party, state_pv2p, ics, incumbent, poll_week_4, state_pv2p_lag, mean_support)
Maryland <- Maryland |> select(year, state, party, state_pv2p, ics, incumbent, poll_week_4, state_pv2p_lag, mean_support)
Massachusetts <- Massachusetts |> select(year, state, party, state_pv2p, ics, incumbent, poll_week_4, state_pv2p_lag, mean_support)
Michigan <- Michigan |> select(year, state, party, state_pv2p, ics, incumbent, poll_week_4, state_pv2p_lag, mean_support)
Minnesota <- Minnesota |> select(year, state, party, state_pv2p, ics, incumbent, poll_week_4, state_pv2p_lag, mean_support)
Mississippi <- Mississippi |> select(year, state, party, state_pv2p, ics, incumbent, poll_week_4, state_pv2p_lag, mean_support)
Missouri <- Missouri |> select(year, state, party, state_pv2p, ics, incumbent, poll_week_4, state_pv2p_lag, mean_support)
Montana <- Montana |> select(year, state, party, state_pv2p, ics, incumbent, poll_week_4, state_pv2p_lag, mean_support)
Nebraska <- Nebraska |> select(year, state, party, state_pv2p, ics, incumbent, poll_week_4, state_pv2p_lag, mean_support)
Nevada <- Nevada |> select(year, state, party, state_pv2p, ics, incumbent, poll_week_4, state_pv2p_lag, mean_support)
`New Hampshire` <- `New Hampshire` |> select(year, state, party, state_pv2p, ics, incumbent, poll_week_4, state_pv2p_lag, mean_support)
`New Jersey` <- `New Jersey` |> select(year, state, party, state_pv2p, ics, incumbent, poll_week_4, state_pv2p_lag, mean_support)
`New Mexico` <- `New Mexico` |> select(year, state, party, state_pv2p, ics, incumbent, poll_week_4, state_pv2p_lag, mean_support)
`New York` <- `New York` |> select(year, state, party, state_pv2p, ics, incumbent, poll_week_4, state_pv2p_lag, mean_support)
`North Carolina` <- `North Carolina` |> select(year, state, party, state_pv2p, ics, incumbent, poll_week_4, state_pv2p_lag, mean_support)
`North Dakota` <- `North Dakota` |> select(year, state, party, state_pv2p, ics, incumbent, poll_week_4, state_pv2p_lag, mean_support)
Ohio <- Ohio |> select(year, state, party, state_pv2p, ics, incumbent, poll_week_4, state_pv2p_lag, mean_support)
Oklahoma <- Oklahoma |> select(year, state, party, state_pv2p, ics, incumbent, poll_week_4, state_pv2p_lag, mean_support)
Oregon <- Oregon |> select(year, state, party, state_pv2p, ics, incumbent, poll_week_4, state_pv2p_lag, mean_support)
Pennsylvania <- Pennsylvania |> select(year, state, party, state_pv2p, ics, incumbent, poll_week_4, state_pv2p_lag, mean_support)
`Rhode Island` <- `Rhode Island` |> select(year, state, party, state_pv2p, ics, incumbent, poll_week_4, state_pv2p_lag, mean_support)
`South Carolina` <- `South Carolina` |> select(year, state, party, state_pv2p, ics, incumbent, poll_week_4, state_pv2p_lag, mean_support)
`South Dakota` <- `South Dakota` |> select(year, state, party, state_pv2p, ics, incumbent, poll_week_4, state_pv2p_lag, mean_support)
Tennessee <- Tennessee |> select(year, state, party, state_pv2p, ics, incumbent, poll_week_4, state_pv2p_lag, mean_support)
Texas <- Texas |> select(year, state, party, state_pv2p, ics, incumbent, poll_week_4, state_pv2p_lag, mean_support)
Utah <- Utah |> select(year, state, party, state_pv2p, ics, incumbent, poll_week_4, state_pv2p_lag, mean_support)
Vermont <- Vermont |> select(year, state, party, state_pv2p, ics, incumbent, poll_week_4, state_pv2p_lag, mean_support)
Virginia <- Virginia |> select(year, state, party, state_pv2p, ics, incumbent, poll_week_4, state_pv2p_lag, mean_support)
Washington <- Washington |> select(year, state, party, state_pv2p, ics, incumbent, poll_week_4, state_pv2p_lag, mean_support)
`West Virginia` <- `West Virginia` |> select(year, state, party, state_pv2p, ics, incumbent, poll_week_4, state_pv2p_lag, mean_support)
Wisconsin <- Wisconsin |> select(year, state, party, state_pv2p, ics, incumbent, poll_week_4, state_pv2p_lag, mean_support)
Wyoming <- Wyoming |> select(year, state, party, state_pv2p, ics, incumbent, poll_week_4, state_pv2p_lag, mean_support)
...
Alabama24 <- Alabama24 |> select(year, state, party, state_pv2p, ics, incumbent, poll_week_4, state_pv2p_lag, mean_support)
Alaska24 <- Alaska24 |> select(year, state, party, state_pv2p, ics, incumbent, poll_week_4, state_pv2p_lag, mean_support)
Arkansas24 <- Arkansas24 |> select(year, state, party, state_pv2p, ics, incumbent, poll_week_4, state_pv2p_lag, mean_support)
Arizona24 <- Arizona24 |> select(year, state, party, state_pv2p, ics, incumbent, poll_week_4, state_pv2p_lag, mean_support)
California24 <- California24 |> select(year, state, party, state_pv2p, ics, incumbent, poll_week_4, state_pv2p_lag, mean_support)
Colorado24 <- Colorado24 |> select(year, state, party, state_pv2p, ics, incumbent, poll_week_4, state_pv2p_lag, mean_support)
Connecticut24 <- Connecticut24 |> select(year, state, party, state_pv2p, ics, incumbent, poll_week_4, state_pv2p_lag, mean_support)
Delaware24 <- Delaware24 |> select(year, state, party, state_pv2p, ics, incumbent, poll_week_4, state_pv2p_lag, mean_support)
Florida24 <- Florida24 |> select(year, state, party, state_pv2p, ics, incumbent, poll_week_4, state_pv2p_lag, mean_support)
Georgia24 <- Georgia24 |> select(year, state, party, state_pv2p, ics, incumbent, poll_week_4, state_pv2p_lag, mean_support)
Hawaii24 <- Hawaii24 |> select(year, state, party, state_pv2p, ics, incumbent, poll_week_4, state_pv2p_lag, mean_support)
Idaho24 <- Idaho24 |> select(year, state, party, state_pv2p, ics, incumbent, poll_week_4, state_pv2p_lag, mean_support)
Illinois24 <- Illinois24 |> select(year, state, party, state_pv2p, ics, incumbent, poll_week_4, state_pv2p_lag, mean_support)
Indiana24 <- Indiana24 |> select(year, state, party, state_pv2p, ics, incumbent, poll_week_4, state_pv2p_lag, mean_support)
Iowa24 <- Iowa24 |> select(year, state, party, state_pv2p, ics, incumbent, poll_week_4, state_pv2p_lag, mean_support)
Kansas24 <- Kansas24 |> select(year, state, party, state_pv2p, ics, incumbent, poll_week_4, state_pv2p_lag, mean_support)
Kentucky24 <- Kentucky24 |> select(year, state, party, state_pv2p, ics, incumbent, poll_week_4, state_pv2p_lag, mean_support)
Louisiana24 <- Louisiana24 |> select(year, state, party, state_pv2p, ics, incumbent, poll_week_4, state_pv2p_lag, mean_support)
Maine24 <- Maine24 |> select(year, state, party, state_pv2p, ics, incumbent, poll_week_4, state_pv2p_lag, mean_support)
Maryland24 <- Maryland24 |> select(year, state, party, state_pv2p, ics, incumbent, poll_week_4, state_pv2p_lag, mean_support)
Massachusetts24 <- Massachusetts24 |> select(year, state, party, state_pv2p, ics, incumbent, poll_week_4, state_pv2p_lag, mean_support)
Michigan24 <- Michigan24 |> select(year, state, party, state_pv2p, ics, incumbent, poll_week_4, state_pv2p_lag, mean_support)
Minnesota24 <- Minnesota24 |> select(year, state, party, state_pv2p, ics, incumbent, poll_week_4, state_pv2p_lag, mean_support)
Mississippi24 <- Mississippi24 |> select(year, state, party, state_pv2p, ics, incumbent, poll_week_4, state_pv2p_lag, mean_support)
Missouri24 <- Missouri24 |> select(year, state, party, state_pv2p, ics, incumbent, poll_week_4, state_pv2p_lag, mean_support)
Montana24 <- Montana24 |> select(year, state, party, state_pv2p, ics, incumbent, poll_week_4, state_pv2p_lag, mean_support)
Nebraska24 <- Nebraska24 |> select(year, state, party, state_pv2p, ics, incumbent, poll_week_4, state_pv2p_lag, mean_support)
Nevada24 <- Nevada24 |> select(year, state, party, state_pv2p, ics, incumbent, poll_week_4, state_pv2p_lag, mean_support)
NH24 <- NH24 |> select(year, state, party, state_pv2p, ics, incumbent, poll_week_4, state_pv2p_lag, mean_support)
NJ24 <- NJ24 |> select(year, state, party, state_pv2p, ics, incumbent, poll_week_4, state_pv2p_lag, mean_support)
NM24 <- NM24 |> select(year, state, party, state_pv2p, ics, incumbent, poll_week_4, state_pv2p_lag, mean_support)
NY24 <- NY24 |> select(year, state, party, state_pv2p, ics, incumbent, poll_week_4, state_pv2p_lag, mean_support)
NC24 <- NC24 |> select(year, state, party, state_pv2p, ics, incumbent, poll_week_4, state_pv2p_lag, mean_support)
ND24 <- ND24 |> select(year, state, party, state_pv2p, ics, incumbent, poll_week_4, state_pv2p_lag, mean_support)
Ohio24 <- Ohio24 |> select(year, state, party, state_pv2p, ics, incumbent, poll_week_4, state_pv2p_lag, mean_support)
Oklahoma24 <- Oklahoma24 |> select(year, state, party, state_pv2p, ics, incumbent, poll_week_4, state_pv2p_lag, mean_support)
Oregon24 <- Oregon24 |> select(year, state, party, state_pv2p, ics, incumbent, poll_week_4, state_pv2p_lag, mean_support)
Pennsylvania24 <- Pennsylvania24 |> select(year, state, party, state_pv2p, ics, incumbent, poll_week_4, state_pv2p_lag, mean_support)
RI24 <- RI24 |> select(year, state, party, state_pv2p, ics, incumbent, poll_week_4, state_pv2p_lag, mean_support)
SC24 <- SC24 |> select(year, state, party, state_pv2p, ics, incumbent, poll_week_4, state_pv2p_lag, mean_support)
SD24 <- SD24 |> select(year, state, party, state_pv2p, ics, incumbent, poll_week_4, state_pv2p_lag, mean_support)
Tennessee24 <- Tennessee24 |> select(year, state, party, state_pv2p, ics, incumbent, poll_week_4, state_pv2p_lag, mean_support)
Texas24 <- Texas24 |> select(year, state, party, state_pv2p, ics, incumbent, poll_week_4, state_pv2p_lag, mean_support)
Utah24 <- Utah24 |> select(year, state, party, state_pv2p, ics, incumbent, poll_week_4, state_pv2p_lag, mean_support)
Vermont24 <- Vermont24 |> select(year, state, party, state_pv2p, ics, incumbent, poll_week_4, state_pv2p_lag, mean_support)
Virginia24 <- Virginia24 |> select(year, state, party, state_pv2p, ics, incumbent, poll_week_4, state_pv2p_lag, mean_support)
Washington24 <- Washington24 |> select(year, state, party, state_pv2p, ics, incumbent, poll_week_4, state_pv2p_lag, mean_support)
WV24 <- WV24 |> select(year, state, party, state_pv2p, ics, incumbent, poll_week_4, state_pv2p_lag, mean_support)
Wisconsin24 <- Wisconsin24 |> select(year, state, party, state_pv2p, ics, incumbent, poll_week_4, state_pv2p_lag, mean_support)
Wyoming24 <- Wyoming24 |> select(year, state, party, state_pv2p, ics, incumbent, poll_week_4, state_pv2p_lag, mean_support)
...
# Run model. 
mod_lm_dem <- lm(state_pv2p ~ ics + incumbent + poll_week_4 + state_pv2p_lag + mean_support, 
                 data = d_train)
summary(mod_lm_dem)

# Simple simulation. 
simp.vars <- c("ics", "incumbent", "poll_week_4", "state_pv2p_lag", "mean_support")
mod_lm_dem_simp <- lm(state_pv2p ~ ics + incumbent + poll_week_4 + state_pv2p_lag + mean_support,
                      data = d_train)
...
```{r ec numeric, echo=FALSE, include=FALSE, warning=FALSE}
win_pred <- win_pred |>
  mutate(
...
sum(win_pred$harris_ec) # 267 electoral college votes for Harris
```
...
pred.mat <- data.frame(state = rep(d_test_simp$state, m),
                       ...
                       mean_support = rep(d_train$mean_support, m),
                       simp_pred_dem = rep(simp_pred_dem, m))

j <- 1
for (i in 1:m) {
  print(i)
  mean_support <- sapply(d_test$mean_support, function(mu) {
    rnorm(1, mean = mu, sd = 0.05) # Simulate turnout from Gaussian centered on national average with 5% SD.
  })

  d_test_samp <- d_test_simp
  d_test_samp$mean_support <- mean_support
...
pred.mat <- pred.mat |>
  mutate(winner = ifelse(simp_pred_dem > 50, "Democrat", "Republican"),
         harris_win = ifelse(simp_pred_dem > 50, 1, 0))

pred.mat |>
  group_by(state) |>
  summarize(win_rate = mean(harris_win)) |> view()
  mutate(
    region = tolower(state)
  ) |>
  right_join(states_map, by = "region") |>
  ggplot(aes(long, lat, group = group)) +
  geom_polygon(aes(fill = win_rate), color = "#000033") +
  scale_fill_gradient2(high = "#7CB9E8", # high (positive) values represent a greater win rate
                       mid = "#E8747C", # low (negative) values represent a lower Harris 
                       name = "win rate",
                       breaks = c(0, .25, .50, .25, 1),
                       limits = c(0, 1)) +
  ggtitle("Fig V. Electoral College Prediction, 2024") +
  my_map_theme() +
  theme(strip.text = element_text(size = 12), 
