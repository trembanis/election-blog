...
party_econ |> 
  ...
  labs(x = "Second Quarter Disposable Personal Income", 
       y = "Popular Vote Share - Incumbent Party (%)", 
       title = "Fig. VII - Q2 DPI vs. Vote Share") + 
  my_theme() + 
  theme(plot.title = element_text(size = 18))
```
...
party_econ |> 
  ...
  labs(x = "Second Quarter Consumer Price Index", 
       y = "Popular Vote Share - Incumbent Party (%)", 
       title = "Fig. VIII - Q2 CPI vs. Vote Share") + 
  my_theme() + 
  theme(plot.title = element_text(size = 18))
```
...
```{r multi OLS}
multi_with <- lm(pv2p ~ GDP_growth_quarterly + unemployment + CPI + dpi, 
               data = party_econ)
multi_with |> summary()

multi_without <- lm(pv2p ~ GDP_growth_quarterly + unemployment + CPI + dpi,
                    data = party_econ_omit)
...
# use q2 gdp for 2024 to predict next pop vote (2020 omitted)
predict(gdp_without, GDP_new, interval = "prediction")

# use q2 unemployment for 2024 to predict next pop vote (2020 included)
predict(unemp_with, unemp_new, interval = "prediction")

# use q2 dpi for 2024 to predict next pop vote (2020 included)
predict(dpi_with, dpi_new, interval = "prediction")

# use q2 cpi for 2024 to predict next pop vote (2020 included)
predict(cpi_with, cpi_new, interval = "prediction")

# use multi OLS to predict next pop vote (2020 omitted)
