...
pop_acc_plot <- ggplot(data = to_plot, mapping = aes(x=predicted, y=actual, label = year)) +
  ...
  labs(x = "Actual Vote Share",
       ...
       title = "Fig. ? - Accuracy Over Time (Popular Vote)") +
  my_theme()
pop_acc_plot
```
...
fig_er <- results_mapped |>
  ...
  scale_fill_gradient2(high = "#EE2C2C", # high (positive) values represent a bias toward Trump (underestimated Harris)
                       ...
                       limits = c(-5, 5)) +
  ggtitle("Fig. II - State-Level Bias, 2024") +
  my_map_theme() +
  theme(strip.text = element_text(size = 12), 
        aspect.ratio = 0.75)
```
...
rsquared <- tibble(Regression = c("All Variables", "No Polling Average", "No Index of Consumer Sentiment", "No Previous Administration Dummy", "No Lagged Vote Share", "Swap GDP Growth for ICS"),
        `Adjusted R-squared` = c(0.6939, 0.5955, 0.7221, 0.685, 0.6949, 0.7282),
        `2024 Bias (Actual - Predicted)` = c(-1.96415, -0.04213, -2.05357, -3.34127, -2.76374, -2.5103),
        `Mean Squared Error` = c(4.784051, 7.02571, 4.825533, 5.469884, 5.299329, 4.247583),
        `Root Mean Squared Error` = c(2.187247, 2.650606, 2.19671, 2.338778, 2.302027, 4.247583))
